

import { motion } from "framer-motion";
import {
  MdAttachMoney,
  MdLock,
  MdSecurity,
  MdTrendingUp,
} from "react-icons/md";
import { useInView } from "react-intersection-observer";

const cards = [
  {
    title: "Safe Investment",
    desc: "Devshree Venture provides safe investment options with a focus on consistent returns and risk management.",
    icon: <MdAttachMoney size={34} className="text-gray-800" />,
  },
  {
    title: "Best Planning",
    desc: "We focus on investing in various sectors, providing opportunities for growth and diversification.",
    icon: <MdTrendingUp size={34} className="text-gray-800" />,
  },
  {
    title: "Reliable Return",
    desc: "We provide professional investment services to help you grow your equity funds and achieve reliable returns.",
    icon: <MdSecurity size={34} className="text-gray-800" />,
  },
  {
    title: "Trusted & Secure",
    desc: "We prioritize transparency and security for your investments. Trust us to grow your equity funds.",
    icon: <MdLock size={34} className="text-gray-800" />,
  },
];

// Motion Variants
const fadeIn = (direction = "up", delay = 0) => ({
  hidden: {
    opacity: 0,
    x: direction === "left" ? -60 : direction === "right" ? 60 : 0,
    y: direction === "up" ? 40 : 0,
  },
  visible: {
    opacity: 1,
    x: 0,
    y: 0,
    transition: {
      type: "spring",
      stiffness: 260,
      damping: 26,
      delay,
    },
  },
});

const WhyChooseUs = () => {
  const { ref, inView } = useInView({ triggerOnce: true, threshold: 0.2 });

  return (
    <section
      ref={ref}
      className="
        relative overflow-hidden
        py-20 px-5 md:px-10 lg:px-16
        bg-gradient-to-br from-white/95 via-gray-100 to-white/95
        text-gray-800
      "
    >
      {/* LIGHT TOP LINE */}
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-gray-400 to-transparent" />

      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
        {/* TEXT SECTION */}
        <motion.div
          initial="hidden"
          animate={inView ? "visible" : "hidden"}
          variants={{
            hidden: {},
            visible: { transition: { staggerChildren: 0.15 } },
          }}
          className="space-y-6"
        >
          <motion.div
            variants={fadeIn("left")}
            className="flex items-center gap-3"
          >
            <span className="text-sm font-semibold uppercase tracking-widest text-gray-600">
              WHY CHOOSE US
            </span>
            <span className="w-2 h-2 bg-gray-400 rounded-full"></span>
          </motion.div>

          <motion.h2
            variants={fadeIn("left", 0.1)}
            className="text-4xl md:text-5xl font-bold leading-tight text-gray-900"
          >
            Why{" "}
            <span className="relative inline-block text-gray-700">
              Choose
              <span className="absolute -bottom-2 left-0 w-full h-2 bg-gray-300/40 rounded-md"></span>
            </span>{" "}
            Us
          </motion.h2>

          <motion.p
            variants={fadeIn("left", 0.2)}
            className="text-lg text-gray-600 max-w-lg"
          >
            We help our clients grow wealth through disciplined strategies,
            professional planning, and long-term investment vision.
          </motion.p>
        </motion.div>

        {/* CARDS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {cards.map((card, i) => (
            <motion.div
              key={i}
              initial="hidden"
              animate={inView ? "visible" : "hidden"}
              variants={fadeIn("right", i * 0.1)}
              whileHover={{
                y: -10,
                boxShadow: "0 20px 35px rgba(0,0,0,0.1)",
              }}
              className="
                bg-white/90
                border border-gray-300
                rounded-xl p-6
                transition-all duration-300
              "
            >
              <div className="mb-4">{card.icon}</div>

              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {card.title}
              </h3>

              <p className="text-sm leading-relaxed text-gray-600">
                {card.desc}
              </p>
            </motion.div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default WhyChooseUs;
