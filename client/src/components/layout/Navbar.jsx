...
import { AnimatePresence, motion } from "framer-motion";
import { useEffect, useRef, useState } from "react";
import { HiMenu, HiOutlineCalendar, HiOutlineClock, HiX } from "react-icons/hi";
import { Link, NavLink } from "react-router-dom";
import devshreeLogo from "../../assets/logos/devshree.jpeg";
...
const Navbar = () => {
  ...
  useEffect(() => {
    const updateDateTime = () => {
      ...
      setNepaliDate(
        `${bsMonthNames[parseInt(bsMonthNum) - 1]} ${parseInt(bsDay)}, ${bsYear}`
      );
    };
    updateDateTime();
    const interval = setInterval(updateDateTime, 60000);
  ...
  useEffect(() => {
    const move = (e) => {
      setMousePosition({ x: e.clientX, y: e.clientY });
      if (window.innerWidth > 768 && !isMenuOpen && e.clientX < 30) setIsMenuOpen(true);
    };
    if (window.innerWidth > 768) {
      window.addEventListener("mousemove", move);
      return () => window.removeEventListener("mousemove", move);
  ...
  const drawerVariants = {
    closed: { x: "-100%", transition: { type: "spring", stiffness: 320, damping: 35 } },
    open: { x: 0, transition: { type: "spring", stiffness: 260, damping: 26 } },
  };
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full z-50">
        ...
        <nav
          className={`bg-gradient-to-r from-[#FFF9EC] via-[#FAF8F3] to-[#FFF9EC]/90 backdrop-blur-xl border-b border-[#E6D8A5] transition-all ${
            isScrolled ? "shadow-lg shadow-[#D4AF37]/10" : ""
          }`}
        >
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex h-16 items-center justify-between">
              {/* LOGO */}
              <Link className="flex items-center gap-3" to="/">
                <img src={devshreeLogo} alt="logo" className="w-9 h-9 rounded-full" />
                <div className="hidden sm:block">
                  <p className="text-sm font-semibold text-amber-900">
                    Devshree Venture Pvt. Ltd.
                  </p>
                  <p className="text-xs text-[#8B7A44]">Invest for Growth</p>
                </div>
              </Link>
              ...
              <div className="flex items-center gap-4">
                <div className="hidden md:block text-right">
                  <div className="flex items-center gap-2 text-[#8B7A44] text-xs">
                    <HiOutlineCalendar /> {nepaliDate}
                  </div>
                  <div className="flex items-center gap-2 text-[#8B7A44] text-xs mt-1">
                    <HiOutlineClock /> {currentDay}, {currentTime}
                  </div>
                </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <>
            <motion.div
              className="fixed inset-0 z-40 bg-[#FFD700]/10 backdrop-blur-sm"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={closeMenu}
            ...
            >
              <div className="flex justify-end p-4">
                <button
                  onClick={closeMenu}
                  className="p-2 rounded-lg text-amber-900 hover:bg-[#EADFB8]"
                >
                  <HiX size={24} />
                </button>
              </div>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block text-lg font-medium transition ${
                        isActive
                          ? "text-[#B8962E]"
                          : "text-amber-900 hover:text-[#B8962E]"
                      }`
                    }
                  >
                    {item.name}
