...
const PartnersSection = () => {
  ...
  useEffect(() => {
    const fetchHeroData = async () => {
      try {
        ...
        const investData =
          res?.portfolio?.filter((item) => item.invest === true) || [];

        const logosFromBackend = investData
  ...
  return (
    ...
    >
      <Marquee
        speed={60}              // ⬅️ increase for faster motion
        gradient={true}
        gradientColor={[255, 255, 255]}
        gradientWidth={80}
        pauseOnHover={true}
        pauseOnClick={true}
        autoFill={true}         // ⬅️ key for seamless infinite loop
      >
        {data.map((logo, index) => (
          <PartnerCard key={index} logo={logo} index={index} />
        ))}
      </Marquee>
    </div>
  );
